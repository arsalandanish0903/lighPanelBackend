const {Hanging2} = require("../Models/Hanging2");


exports.Hanging2lightsGetById = async (req, res) => {
  try {
    const {id} = req.params
    const light = await Hanging2.findById(id)
...
exports.Hanging2LightsGetAll = async (req, res) => {
  try {
    const lights = await Hanging2.find().sort({sNo:1});
...
exports.Hanging2postAllLights= async (req, res) => {
  try {
    const data = req.body;

    // Using bracket notation to correctly access the fields with spaces
    const result = await Hanging2.insertMany(data);
...
exports.Hanging2updateLightsData = async (req, res) => {
  try {
    ...
    const result = await Hanging2.updateOne({ _id: id }, { $set: updateData });
