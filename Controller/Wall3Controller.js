const { Wall3Light , Wall3Bollards , Wall3Footlamp  } = require("../Models/Wall3");
exports.Wall3BollardsGetAll = async (req, res) => {
  try {
    const bollards = await Wall3Bollards.find().sort({sNo:1});
...
exports.Wall3LightsGetAll = async (req, res) => {
  try {
    const lights = await Wall3Light.find().sort({sNo:1});
...
exports.Wall3FootlamplightsGetById = async (req, res) => {
  try {
    const {id} = req.params
    const light = await Wall3Footlamp.findById(id)
    res.status(200).json({ success: true, data: light });
  } catch (error) {
    console.error("Error fetching data:", error);
    res.status(500).json({ success: false, message: "Server Error" });
...
exports.Wall3FootlampLightsGetAll = async (req, res) => {
  try {
    const lights = await Wall3Footlamp.find().sort({sNo:1});
    res.status(200).json({ success: true, data: lights });
  } catch (error) {
    console.error("Error fetching data:", error);
    res.status(500).json({ success: false, message: "Server Error" });
...
exports.Wall3FootlamppostAllLights= async (req, res) => {
  try {
    const data = req.body;

    // Using bracket notation to correctly access the fields with spaces
    const result = await Wall3Footlamp.insertMany(data);
...
exports.Wall3FootlampupdateLightsData = async (req, res) => {
  try {
    const { id, ...updateData } = req.body; // Extract id separately
    ...
    const result = await Wall3Footlamp.updateOne({ _id: id }, { $set: updateData });
