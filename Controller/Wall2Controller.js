const { Lights, Bollards , Wall2Footlamp  } = require("../Models/Wall2");
...
exports.Wall2FootlamplightsGetById = async (req, res) => {
  try {
    const {id} = req.params
    const light = await Wall2Footlamp.findById(id)
    res.status(200).json({ success: true, data: light });
  } catch (error) {
    console.error("Error fetching data:", error);
    res.status(500).json({ success: false, message: "Server Error" });
...
exports.Wall2FootlampLightsGetAll = async (req, res) => {
  try {
    const lights = await Wall2Footlamp.find().sort({sNo:1});
    res.status(200).json({ success: true, data: lights });
  } catch (error) {
    console.error("Error fetching data:", error);
    res.status(500).json({ success: false, message: "Server Error" });
...
exports.Wall2FootlamppostAllLights= async (req, res) => {
  try {
    const data = req.body;

    // Using bracket notation to correctly access the fields with spaces
    const result = await Wall2Footlamp.insertMany(data);
...
exports.Wall2FootlampupdateLightsData = async (req, res) => {
  try {
    const { id, ...updateData } = req.body; // Extract id separately
    ...
    const result = await Wall2Footlamp.updateOne({ _id: id }, { $set: updateData });
