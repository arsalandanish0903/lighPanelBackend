...
exports.Fan3GetById = async (req, res) => {
  try {
    const {id} = req.params
    const light = await Fan3.findById(id)
...
exports.Fan3LightPost = async (req, res) => {
  try {
    const data = req.body
    const result = await Fan3Light.insertMany(data)
...
exports.Fan3LightupdateBollardsData = async (req, res) => {
  try {
    ...
    const result = await Fan3Light.updateOne({ _id: id }, { $set: updateData });
...
exports.Fan3postAllLights= async (req, res) => {
  try {
    const data = req.body;

    // Using bracket notation to correctly access the fields with spaces
    const result = await Fan3.insertMany(data);
...
exports.Fan3updateLightsData = async (req, res) => {
  try {
    const { id, ...updateData } = req.body; // Extract id separately
